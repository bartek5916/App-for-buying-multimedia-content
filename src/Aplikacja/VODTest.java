...
public class VODTest {
    ...
    static int cena(Koszyk k, String typ) {
        int suma = 0;
        for(Gatunek gatunek : k.getKoszyk()){
            for(Program program : Cennik.cennik.get(typ)){
                if(gatunek.getNazwa().equals(program.getTytul())){
                    suma = suma + (int) program.getCenaKlienta() * gatunek.getLiczbaUrzadzen();
                }
            }
        }
        return suma;
    ...
    public static void main(String[] args) {
        ...
        cennik.dodaj("sensacja", "Król Artur");



        // Aplikacja.Klient Kinoman deklaruje kwotę 60 zł na zamównienia, posiada abonament w serwisie
        Klient kinoman = new Klient("Kinoman", 60, Abonament.TAK);

        // Aplikacja.Klient Kinoman dodaje do listy życzeń różne programy:
        ...
        System.out.println("Progamy obyczajowe w koszyku klienta Kinoman kosztowały:  " + cena(koszykKinomana, "obyczaj"));

        // Aplikacja.Klient zapłaci...
        ...
        Klient krytyk = new Klient("Krytyk", 60, Aplikacja.Abonament.NIE);

        // Zamówił za dużo jak na tę kwotę
        krytyk.dodaj(new Dramat("Król lew", 1));
        krytyk.dodaj(new Komedia("Shrek", 3));
        krytyk.dodaj(new Komedia("Królowa", 3));
        krytyk.dodaj(new Obyczaj("Król Lear", 3));
        krytyk.dodaj(new Dramat("Król Lear", 3));
